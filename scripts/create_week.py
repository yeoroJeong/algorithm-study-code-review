#!/usr/bin/env python3
"""Create week.yml, README and all member/problem submission folders."""
...
def create_week(week_number: int, start: str, end: str, deadline: str, problems_file: Path, force: bool) -> list[Path]:
    week = normalized_week(week_number)
    week_dir = ROOT / "problems" / week
    config_path = week_dir / "week.yml"
    ...
    config: dict[str, Any] = {
        ...
        "submission": {"root": "solutions", "extensions": [".py", ".java", ".cpp"]},
        "notes": (
    ...
    readme.write_text(render_readme(week_dir, load_config(config_path)), encoding="utf-8", newline="\n")
...
def main() -> int:
    ...
    try:
        paths = create_week(args.week, args.start, args.end, args.deadline, args.problems_file, args.force)
